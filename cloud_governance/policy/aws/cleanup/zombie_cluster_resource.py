...
class ZombieClusterResource(ZombieClusterOperations, ABC):
    ...
    def run_zombie_cluster_pruner(self):
        ...
        if self.zombie_cluster_resource_name:
            zombie_clusters_resources = [eval(f"self.{self.zombie_cluster_resource_name}")]
            if eval(f"self.{self.zombie_cluster_resource_name}") in dependencies:
                zombie_clusters_resources = dependencies[eval(f"self.{self.zombie_cluster_resource_name}")]
        zombie_cluster_response = {}
        for zombie_cluster in zombie_clusters_resources:
            if isinstance(zombie_cluster, dict):
                scan_zombie_cluster_resources = []
                for key, resources in zombie_cluster.items():
                    scan_zombie_cluster_resources.extend(list(resources))
            else:
                scan_zombie_cluster_resources = [zombie_cluster]
            for exec_cluster in scan_zombie_cluster_resources:
                ...
                for zombie_tag, zombie_resources in zombie_items.items():
                    if zombie_tag in zombie_cluster_response:
                        ...
                        zombie_cluster_response[zombie_tag].setdefault('ResourceNames', []).append(
                            zombie_cluster.__name__)
